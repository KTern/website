...
export default function PartnerSolutionLandingPage({
  pageData,
  headerData,
  footerData,
  stream,
  ...
  return (
    <>
      ...
      <Layout h_data={headerData} f_data={footerData}>
        ...
        <MultiCardCarousel
          data={pageData.ProductOfferings}
          stream={stream}
        ></MultiCardCarousel>
        <PricingCard
          stream={stream}
...
export const getServerSideProps = async (ctx) => {
  ...
  const res = await fetch(
    `https://strapi.ktern.com/partner-solutions?PartnerSlug=${partnerslug}&SolutionSlug=${solutionslug}`,
    {
      method: "get",
    }
  );
