...
export default function ValueAssesment({ h_data, f_data }) {
  ...
  useEffect(() => {
    ...
    return () => {
      document
        .getElementById("button_field")
        .removeEventListener("click", changeSlide);
    };
  }, []);
  // Change Slide
  ...
  const renderView = ({ index, active, transitionState }) => {
    console.log(index, "==", keyValue, question.length);
    data = question[index];
    switch (data.type) {
      case "Textbox":
        return (
          <div className=" text-white flex flex-col events   items-center justify-center ">
            ...
            >
              {" "}
              Assess Your Customer Service Maturity
            </h1>
            <input
              type="text"
              id="field"
        ...
        return (
          <div className=" text-white flex flex-col events  items-center justify-center ">
            ...
            <p className="subheading w-2/4 text-center">
              Find out if your processes still need to be optimized, if you’re
              using the latest customer service technology, and most importantly
              - if you are delivering memorable customer experiences.
            </p>
          </div>
        );
    }
  ...
  return (
    <>
      ...
      <Layout h_data={h_data} f_data={f_data}>
        <div
          className="bg-valueBg events bg-cover text-white flex flex-col space-y-16 items-center justify-center overflow-hidden"
          style={{ height: "100vh" }}
        >
          {keyValue < question.length && (
            <ViewSlider
          ...
          <button
            id="button_field"
            className=" bg-opacity-40 hover:bg-opacity-40 sm:mb-4 border-2 border-white  inline-block py-3 px-2 md:px-10 bg-black hover:bg-gray-300 hover:text-black shadow   text-white  rounded-r-xl rounded-b-xl transition duration-200 button "
          >
            Start Quiz
          </button>
