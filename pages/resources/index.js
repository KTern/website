          ...
          <div className="relative z-20 flex flex-col sm:px-4 md:w-2/3 h-full ">
            ...
            <div className="md:flex md:flex-row md:space-x-4 sm:space-y-4">
              ...
              <Link
                ...
                className=" sm:mb-4 inline-block  mr-4 border-2 border-black shadow-md py-3 px-14 bg-white  hover:bg-gray-300   text-black   rounded-r-xl rounded-b-xl transition duration-200 uppercase border-2 border-black button ">
          ...
          <select
            onChange={() => {
              onClick({
                ...
                section_name: "Filter Section-Streams",
                page_source: `${data.PageSEO.PageTitle}`,
                label: `${document.getElementById("stream").value}`,
              });
            }}
            name="stream"
            id="stream"
          ...
          <select
            onChange={() => {
              onClick({
                stream_score: resolve_stream_score("none"),
                event_name: "Filter Click",
                section_name: "Filter Section-Topic",
                page_source: `${data.PageSEO.PageTitle}`,
          ...
          <select
            onChange={() => {
              onClick({
                stream_score: resolve_stream_score("none"),
                event_name: "Filter Click",
                section_name: "Filter Section-Role",
                page_source: `${data.PageSEO.PageTitle}`,
                label: `${document.getElementById("role").value}`,
              });
            }}
            name="role"
            id="role"
          ...
          <select
            onChange={() => {
              onClick({
                stream_score: resolve_stream_score("none"),
                event_name: "Filter Click",
                section_name: "Filter Section-Type",
                page_source: `${data.PageSEO.PageTitle}`,
            ...
            <div className="xl:gap-6 gap-3 grid sm:grid-cols-1 w-full grid-cols-2 xl:grid-cols-3 p-3 mx-auto xl:p-6">
              {ResourceList.slice(0)
                ...
                .map((dt) => (
                  ...
                  >
                    <span
                      className={`absolute top-0 left-0 h-full mt-1 ml-1 bg-secondary  group-hover:bg-${dt.DigitalStream}-secondary `}
                      style={{ height: "400px", width: "345px" }}
                    ></span>
                    <div
                      className={`relative shadow bg-white flex-col p-5 px-5  group overflow-hidden false border border-black hover:border-${dt.DigitalStream}-primary `}
                    >
                      <div className="">
                        <Link
                          href={dt.ResourceURL}
                            ...
                            <div>
                              <div className="absolute inset-0 z-10 flex items-center justify-center mb-6">
                                <span className="flex text-white items-center justify-center w-12 h-12 bg-black rounded-full group-hover:text-black shadow-2xl group-hover:bg-white opacity-60">
                                  <svg
                                    className="w-auto h-6 ml-1   fill-current"
                                    viewBox="0 0 52 66"
