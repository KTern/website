...
export default function FeaturedResourcesComponent({
  resources,
  stream,
  articles,
  news,
      ...
      <div className="max-w-6xl  mx-auto pb-12">
          ...
          {resources.MajorResourceCards.map((dt) => (
            ...
            >
              ...
              >
                ...
                <p className="mb-2 text-md font-normal text-purple-100 opacity-100">
                  {dt.Description}
                </p>
              </a>
            </div>
        ...
        <div className="grid grid-cols-12 gap-10 mt-10">
          <div className="col-span-12 md:col-span-8 sm:col-span-10">
            <div
              className={`relative overflow-hidden border-b-2 border-${stream} `}
            >
              <h4
            ...
            {articles.ResourcesList.map((dt1) => (
              <div
                key="dt1"
                className={`flex items-center py-5 border-b border-${stream}`}
              >
                <a
                  className={'w-1/3 h-40 overflow-hidden bg-gray-400 '}
                  href={dt1.PageURL}
                >
                  <img
                ...
                <a className="w-2/3 pl-5" href={dt1.PageURL}>
                  ...
                  <p className="text-sm text-white opacity-50 hover:opacity-100">
                    {dt1.Description}
                  </p>
                  <span className="relative flex mt-3 text-xs text-white opacity-90">
                    <span
                      className={`mr-1 font-normal text-xs p-2 border border-coolGray-200 border border-${stream} `}
                    >
                      {dt1.Tag}
                    </span>
                  </span>
          ...
          <div className="col-span-12 md:col-span-4 sm:col-span-10 sm:p-8">
            ...
            {news.ResourcesList.map((dt2) => (
              <div key="dt2">
                <a
                  href={dt2.PageURL}
                  className="relative block w-full h-64 mt-5 overflow-hidden"
                >
                  <div
                    className={`absolute inset-0 w-full h-full bg-${stream} opacity-90 `}
                  />
                  <img
                ...
                <a className="w-2/3 pl-5" href={dt2.PageURL}>
				  ...
				  opacity-50 hover:opacity-100">{dt2.Description}</p> */}
                  <span className="relative flex mt-3 text-xs text-white opacity-90">
                    <span
                      className={`mr-1 font-normal text-xs p-2 border border-coolGray-200 border border-${stream}`}
                    >
